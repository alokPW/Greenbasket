...
const ProductCart = ({ product }) => {
  const { setCount, currency, addToCart, removeFromCart, cartItems, navigate } =
    useAppContext();
            ...
            {Array(5)
              ...
              .map((_, i) => (
                <img
                  ...
                  scr={i < 4 ? assets.search_icon : assets.star_dull_icon}
                ></img>
              ))}
            <p>(4)</p>
          </div>
            ...
            >
              {!cartItems[product._id] ? (
                ...
                <div className="flex items-center justify-center gap-2 md:w-20 w-16 h-[34px] bg-primary/25 rounded select-none">
                  ...
                  {/* // replace by me */}
                  {/* <button
                  onClick={() => setCount((prev) => addToCart(product._id))}
                  className="cursor-pointer text-md px-2 h-full"
                  >
                  +
                 </button> */}
